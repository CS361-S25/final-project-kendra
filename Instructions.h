...
struct RetrieveMessageInstruction { //struct is similar to class but everything is default public
    ...
    static void run(sgpl::Core<Spec> &core, const sgpl::Instruction<Spec> &inst,
        ...
        state.world->RetrieveMessageHelper(state.current_location);

    }

    static std::string name() { return "Retrieve Message"; } //Whatever you want the human readable name to be
    static size_t prevalence() { return 1; } //How prevalent you want this instruction to be relative to others, 1 is a good value
};
...
using Library =
    ...
    sgpl::Add, sgpl::Subtract, sgpl::global::JumpIfNot, sgpl::local::JumpIfNot, sgpl::global::Anchor, IOInstruction, ReproduceInstruction, NandInstruction, SendMessageInstruction, RetrieveMessageInstruction>;
